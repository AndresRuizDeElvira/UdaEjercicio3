...
UKF::UKF() {
  ...
  */
  //We initialize values following those of the lesson 5, 7 for augmented points and 
  //lambda=3
...
void UKF::ProcessMeasurement(MeasurementPackage meas_package) {
  ...
  */
  ...
  //conversion to coordinates px and py with cosine and sine, using model rho/phi and rho_dot
  	  px = rho * cos(phi);
      py = rho * sin(phi);
...
void UKF::Prediction(double delta_t) {
  ...
  */
  ...
  //we use some helper functions as follows. Credit goes to my tutor who suggested
  //most of the code for this part/istephanov
...
//we generate the sigmaPoints and then use delta_t to predict possible location of sigma points
  ...
  //Predict mean by multiplying weights..
   ...
   double weight1=lambda_ / (lambda_ + n_aug_);
...
// while (x_diff(3)<-M_PI) x_diff(3)+=2.*M_PI;
MatrixXd temp1= mean_subtracted1 * weight2;
...
//of i++.
  temp1.col(0) = temp1.col(0) / weight2 * weight1;
...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
  ...
  */
    //most of the code this function  was suggested by my tutor based on the code of a fellow student
    //had some trouble calculating it.
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  ...
  */
  //We use same measurements as in the case of LIDAR we declare them
    int n_z = 3;
